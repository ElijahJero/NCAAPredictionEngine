"""
NCAA Men's College Basketball Tournament Prediction Tool
GUI-based with Monte Carlo simulation, stat weight customization, and advanced reporting.
...
DEFAULT_WEIGHTS = {
    "Offense (PPG)":          ("avgPoints",                0.35, True),
    "Defense (Opp PPG)":      ("avgPointsAllowed",         0.35, False),
    "Rebounds (RPG)":         ("avgRebounds",              0.20, True),
    "Turnovers (TOPG)":       ("avgTurnovers",             0.10, False),
    "Assists (APG)":          ("avgAssists",               0.00, True),
    "FG% (Field Goal %)":     ("fieldGoalPct",             0.00, True),
    "3PT% (Three Point %)":   ("threePointFieldGoalPct",   0.00, True),
    "FT% (Free Throw %)":     ("freeThrowPct",             0.00, True),
    "Steals (SPG)":           ("avgSteals",                0.00, True),
    "Blocks (BPG)":           ("avgBlocks",                0.00, True),
}
...
class NCAAApp(tk.Tk):
    ...
    def _build_teams_tab(self):
        ...
        cols = ("Team", "PPG", "Opp PPG", "RPG", "TOPG",
                "APG", "FG%", "3PT%", "FT%", "SPG", "BPG", "Strength")
        self.teams_tree = ttk.Treeview(tab, columns=cols,
                                       show="headings", selectmode="browse")
        col_w = {"Team": 200, "PPG": 65, "Opp PPG": 75, "RPG": 65,
                 "TOPG": 65, "APG": 65, "FG%": 65, "3PT%": 65,
                 "FT%": 65, "SPG": 65, "BPG": 65, "Strength": 90}
        for c in cols:
    ...
    def _refresh_advanced(self):
        ...
        # Use the corrected ESPN API stat key names.
        stat_keys = [
            ("avgPoints",    "PPG"),
            ("avgPointsAllowed", "Opp PPG"),
            ("avgRebounds",  "RPG"),
            ("avgTurnovers", "TOPG"),
        ]
    ...
    def _populate_teams_tree(self):
        ...
        for t in self.teams:
            ...
            self.teams_tree.insert("", "end", values=(
                ...
                round(s.get("avgPointsAllowed", 0), 1),   # may be 0 if not in API
